...
namespace SheddingCardGame.UI.Pages
{
    public class InGameUiBuilder
    {
        ...
        public async Task<UiState> Build(GameController gameController)
        {
            ...
            {
                var cardGameObject = new CardComponent(gameController, card,
                    new Sprite(cardsSpriteSheet, new Size(cardWidth, 240), new Point(deckX, deckY)), false);
                uiState.CardGameObjects.Add(card, cardGameObject);
                deckX += 30;
                counter++;
                if (counter == 13)
                {
        ...
        public void SetupStatusArea(GameController gameController, Point statusAreaPosition)
        {
            ...
            uiState.TakeButton = new ButtonComponent(gameController, "Take", new Point(statusAreaPosition.X, y), true);
            uiState.GameObjects.Add(uiState.TakeButton);
            y += rowHeight;

            var x = statusAreaPosition.X + 500;
            y = statusAreaPosition.Y;
            uiState.ClubsButton = new ButtonComponent(gameController, "Clubs", new Point(x, y), true);
            uiState.GameObjects.Add(uiState.ClubsButton);
            y += rowHeight;
            
            uiState.DiamondsButton = new ButtonComponent(gameController, "Diamonds", new Point(x, y), true);
            uiState.GameObjects.Add(uiState.DiamondsButton);
            y += rowHeight;
            
            uiState.HeartsButton = new ButtonComponent(gameController, "Hearts", new Point(x, y), true);
            uiState.GameObjects.Add(uiState.HeartsButton);
            y += rowHeight;
            
            uiState.SpadesButton = new ButtonComponent(gameController, "Spades", new Point(x, y), true);
